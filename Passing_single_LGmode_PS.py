''' Моделирование расспространения атмоферного канала с использованием ОУМ в модах LG и использованием фазовых
экранов '''
...
def Field(l1, p1, lam, X_size, Y_size, x_shift, y_shift, L):  # преобразование LG-пучка в турбулентной среде Field(OAM, x_shift, y_shift, lenghtOfChannel)
    ...
    # собираем всё и нормируем на максимум
    u = C1 * L1_final * q2 * q3 * q4 * q5 * q6 * q7
    u = u / np.max(np.abs(u))
    return u
...
l1 = int(input('Введите азимутальный индекс l:')) #OAM   первый
p1 = int(input('Введите радиальный индекс p:'))          #радиальное распределение интенсивности в пучках LG
...
# #генерируем фазовый экран
Out1 = AIR_phase_screen(lam, L, C_n_21, X_size, Y_size, delta1, L0, l0)

Final_hologram = Field(l1, p1, lam, X_size, Y_size, x_shift, y_shift, L) # распределения поля начального состояния пучка
